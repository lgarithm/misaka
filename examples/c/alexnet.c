...
s_model_t *cnn(shape_t *image_shape, uint8_t arity)
{
    ...
    s_layer_t *c3_c4 = new_layer_conv_nhwc(make_shape_list((p_shape_t[]){
        mk_shape(sc, 3, 3, 3, 384),
        // padding same
        NULL,
    }));
    s_layer_t *c5 = new_layer_conv_nhwc(make_shape_list((p_shape_t[]){
        mk_shape(sc, 3, 3, 3, 256),
        // padding same
        NULL,
    }));
    s_layer_t *f4096 = new_layer_dense(make_shape_list((p_shape_t[]){
        mk_shape(sc, 1, 4096),
        NULL,
    }));
    s_layer_t *f_out = new_layer_dense(make_shape_list((p_shape_t[]){
        mk_shape(sc, 1, arity),
        NULL,
    }));
    s_layer_t *pool = new_layer_pool_max(make_shape_list((p_shape_t[]){
        // 3, 3, stride(2, 2)
        NULL,
    }));                                    //
...
int main()
{
    ...
    s_model_t *model = cnn(image_shape, k);
